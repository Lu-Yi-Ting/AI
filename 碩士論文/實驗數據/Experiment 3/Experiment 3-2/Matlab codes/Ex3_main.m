...
execute=10;
Iteration=1000;
file={'^HSI(2018).csv','^N225(2018).csv','000001.SS(2018).csv','^DJI(2018).csv','2018'};
% file={'^HSI(2018).csv','HSI'
...
for z=1:size(file,1)
    ...
    for t=1:nTarget
        ...
        for i=1:2
            ...
            title(['2018',filename(t,2),'Probability density distributions of features and targets']);
            xlabel('X');
            ylabel('Probability');
            hold on;
        ...
        saveas(fig,['Ex3-2_2018',filename{t,2},'_f',int2str(FP(1)),'f',int2str(FP(2)),'_pd.fig']);
    end
    hold off;
    %% Experiment
    for p=1:execute
        ...
        if nTarget>1
            for t=1:nOutput
                ...
                title(['2018',filename{i,2},' Stock price Prediction']);
                ...
                hold off;
                saveas(fig,['Ex3-2_2018_',filename{i,2},'_Stock price Prediction_',int2str(p),'.fig']);
                
                fig=figure();
                plot(Close2{i+1});
                title(['2018',filename{i+1,2},' Stock price Prediction']);
                xlabel('Trading date index');
                ylabel('Close price');
                hold on;
                ...
                legend('Target function','Testing line','Proposed approach');
                hold off;
                saveas(fig,['Ex3-2_2018_',filename{i+1,2},'_Stock price Prediction_',int2str(p),'.fig']);
                i=i+2;
            end
        else
            fig=figure();
        ...
        for i=1:size(fitness,2)
            ...
            title(['2018',filename{i,2},'Prediction error']);
            ...
            saveas(fig,['Ex3-2_2018_',filename{i,2},'_Prediction error_',int2str(p),'.fig']);
        end
        for i=1:size(fitness,2)
        ...
        xlabel('Iterations');
        ylabel('Cost (RMSE)');
        saveas(fig,['Ex3-2_2018_Learning curve_',int2str(p),'.fig']);
        save(['Ex3-2_2018_4Targets_Workspace',int2str(p)]);
    end
end