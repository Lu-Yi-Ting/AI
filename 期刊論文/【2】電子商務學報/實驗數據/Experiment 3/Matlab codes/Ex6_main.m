...
execute=10;
Iteration=1000;
file={'^HSI(2003).csv','^N225(2003).csv','000001.SS(2003).csv','2000-2003^IXIC.csv','2003'};
 nFeature=14;
Selection='descend';
for z=1:size(file,1)
    ...
    for t=1:nTarget
        ...
        saveas(fig,['Ex6_',file{z,5},filename{t,2},'_f',int2str(FP(1)),'f',int2str(FP(2)),'_pd.fig']);
    end
    ...
    for p=1:execute
        ...
        for i=1:nDim
            ...
            [Train(i).core,Train(i).sigma]=subclust(Train(i).value',0.3);
        end
        %% ContructMatrix
        ...
        if nOutput>=1
            for t=1:nOutput
                ...
                hold off;
                saveas(fig,['Ex6_',file{z,5},filename{i,2},'_Stock price Prediction_',int2str(p),'.fig']);
                ...
                legend('Target function','Testing line','Proposed approach');
                hold off;
                saveas(fig,['Ex6_',file{z,5},filename{i+1,2},'_Stock price Prediction_',int2str(p),'.fig']);
                i=i+2;
            end
        end
        for i=1:size(fitness,2)
            ...
            saveas(fig,['Ex6_',file{z,5},filename{i,2},'_Prediction error_',int2str(p),'.fig']);
        end
        for i=1:size(fitness,2)
        ...
        xlabel('Iterations');
        ylabel('Cost (RMSE)');
        saveas(fig,['Ex6_',file{z,5},'_Learning curve_',int2str(p),'.fig']);
        save(['Ex6_',file{z,5},'_Workspace_',int2str(p)]);
    end
end
