...
execute=10;
Iteration=1000;
filename={'^HSI(2018).csv','HSI'
    '^N225(2018).csv','Nikkei'
    '000001.SS(2018).csv','SSEC'
    '^DJI(2018).csv','DJI'};
%filename={'000001.SS(2018).csv','SSEC'};
num_Feature=[2,4,6,8];
selection={'rand','order'};
    ...
    for z=1:size(num_Feature,2)
        ...
        for t=1:nTarget            
            for i=1:2
                ...
                plot(xpft,pdpft);
                hold on;
                legend(['p_d(t_{',filename{t,2},'})'],['p_d(t_{',filename{t,2},'} | f^-_{',int2str(FP(i)),'})'],['p_d(t_{',filename{t,2},'} | f^+_{',int2str(FP(i)),'})']);
                saveas(fig,['Ex1_2018_',filename{t,2},'_',int2str(nFeature),Selection,'_f',int2str(FP(i)),'_1Targets_pd.fig']);
            end
        ...
        for p=1:execute
            ...
            if nTarget>1
                for t=1:nOutput
                    ...
                    hold off;
                    saveas(fig,['Ex1_2018_',filename{i,2},'_',int2str(nFeature),Selection,'_4Targets_Stock price Prediction_',int2str(p),'.fig']);
                    ...
                    legend('Target function','Testing line','Proposed approach');
                    hold off;
                    saveas(fig,['Ex1_2018_',filename{i+1,2},'_',int2str(nFeature),Selection,'_4Targets_Stock price Prediction_',int2str(p),'.fig']);
                    i=i+2;
                end
            else
                fig=figure();
                ...
                legend('Target function','Testing line','Proposed approach');
                hold off;
                saveas(fig,['Ex1_2018_',filename{i,2},'_',int2str(nFeature),Selection,'_1Target_Stock price Prediction_',int2str(p),'.fig']);
            end
            for i=1:size(fitness,2)
                ...
                saveas(fig,['Ex1_2018_',filename{i,2},'_',int2str(nFeature),Selection,'_4Targets_Prediction error_',int2str(p),'.fig']);
            end
            for i=1:size(fitness,2)
            ...
            xlabel('Iterations');
            ylabel('Cost (RMSE)');
            saveas(fig,['Ex1_2018_',int2str(nFeature),Selection,'_Learning curve_',int2str(p),'.fig']);
            save(['Ex1_2018_',int2str(nFeature),Selection,'_4Targets_Workspace_',int2str(p)]);            
        end
